...
function GridTooltip({ active, payload }: { active?: boolean; payload?: { payload?: Row }[] }) {
  ...
  return (
    <div role="tooltip" className="rounded-lg px-3.5 py-2.5 text-[11px]" style={{ background: "rgba(7,10,20,0.97)", border: `1px solid ${C.green}33`, color: C.text }}>
      ...
      <p className="m-0">Close: <b className="text-white">${fmtPrice(d.close)}</b></p>
      {d.band && (
        <p className="m-0 mt-0.5" style={{ color: C.blue }}>Range: ${fmtPrice(d.band[0])} – ${fmtPrice(d.band[1])}</p>
      )}
    </div>
  );
}
    ...
    <ChartFrame caption={caption} height={200} lgHeight={320} loading={loading}>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={rows} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
          ...
          <YAxis domain={domain} tick={{ fontSize: 9, fill: C.muted }} tickLine={false} axisLine={false} width={58} tickFormatter={formatAxisPrice} />
          ...
          {currentPrice != null && (
            <ReferenceLine y={currentPrice} stroke="rgba(255,255,255,0.7)" strokeDasharray="4 3" label={{ value: fmtPrice(currentPrice), position: "right", fill: "#fff", fontSize: 9 }} />
          )}
